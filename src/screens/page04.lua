...
local animation = require("src.animations.page04.scene01")
...
function scene:create(event)
    ...
    objects.information = information.init("src/assets/instructions.png", "src/assets/page_04/interaction.png")
    sceneGroup:insert(objects.information)

    audio = sound.init("src/assets/page_04/audio.mp3")

    sceneGroup:insert(audio)

    objects.text = display.newImage("src/assets/page_04/text.png")
    objects.text.x = const.WIDTH * (1 / 2)
    objects.text.y = objects.text.width / 2 - (const.HEIGHT * (1 / 32))
