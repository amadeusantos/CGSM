...
local animation = require("src.animations.page03.scene01")
...
function scene:create(event)
    ...
    objects.page = display.newImage("src/assets/pages/page_3.png")
    objects.page.x = const.WIDTH * (1 / 2)
    objects.page.y = const.HEIGHT * (15 / 16)
    sceneGroup:insert(objects.page)

    objects.information = information.init("src/assets/instructions.png", "src/assets/page_03/interaction.png")
    sceneGroup:insert(objects.information)

    audio = sound.init("src/assets/page_03/audio.mp3")

    sceneGroup:insert(audio)

    objects.text = display.newImage("src/assets/page_03/text.png")
    objects.text.x = const.WIDTH * (1 / 2)
    objects.text.y = (const.HEIGHT * (1 / 32)) + (objects.text.width / 2)
