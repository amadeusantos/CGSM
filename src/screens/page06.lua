...
local function nextPage(event)
    composer.gotoScene("src.screens.backCover", "fade")
end
...
function scene:create(event)
    ...
    objects.page = display.newImage("src/assets/pages/page_6.png")
    objects.page.x = const.WIDTH * (1 / 2)
    objects.page.y = const.HEIGHT * (15 / 16)
    sceneGroup:insert(objects.page)

    objects.information = information.init("src/assets/instructions.png", "src/assets/page_06/interaction.png")
    sceneGroup:insert(objects.information)

    audio = sound.init("src/assets/page_06/audio.mp3")

    sceneGroup:insert(audio)

    objects.text = display.newImage("src/assets/page_06/text.png")
